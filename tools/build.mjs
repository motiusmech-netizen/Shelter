...
const FAMILY = { RussoOne: 'Russo One', PTSansNarrow: 'PT Sans Narrow' };
...
function fontFaces() {
  ...
  return readdirSync(dir).filter(f => f.endsWith('.woff2')).sort().map(f => {
    ...
    return `@font-face{font-family:"${FAMILY[fam]}";font-style:normal;font-weight:${weight};font-display:swap;src:url(data:font/woff2;base64,${b64}) format("woff2");unicode-range:${RANGES[subset]};}`;
  }).join('\n');
}
